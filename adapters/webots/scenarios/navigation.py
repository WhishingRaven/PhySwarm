...
PARAMETER_LAYOUT = get_parameter_layout("navigation", contract="legacy")
...
class Epuck2Supervisor(WebotsSupervisorEnv):
    def __init__(self, all_args=None):
        ...
        self.phase_names = get_runtime_macro_spec(
            "navigation", phase_contract=self.phase_contract
        ).phase_names
        ...
        # velocity + density/centroid fields + goal/corridor cues occupy 13 slots;
        # the final phase one-hot is 3-D legacy or 4-D paper contract.
        self.num_observations = 13 + self.num_phases
    ...
    def step(self, action):
        ...
        self._physics_navigation_shape_geometry = None
        ...
        if self._physics_field_bases is not None:
            ...
            info["physics"] = build_physics_transition(
                ...
                # Navigation phase switches are geometry events; the NPC has no
                # lambda channel, so the final axis is intentionally empty.
                reaction_gates=np.zeros(
                    (self.num_envs, self.num_agents, 0), dtype=np.float32
                ),
                auxiliary_fields={
                    "heading_start": control_headings.astype(np.float32),
                    # [centroid-relative x, centroid-relative y, target radius]
                    # is sufficient to rebuild V_shape(beta) inside autograd.
                    "navigation_shape_geometry": self._physics_navigation_shape_geometry,
                },
            )
        ...
        for env_idx in range(self.num_envs):
            ...
            for i in range(self.num_agents):
                ...
                agent_log = {
                    ...
                    'goal_x': float(goal_x),
                    'goal_y': float(goal_y),
                    # S181 needs both the arrival threshold and T_max; recording
                    # them prevents the offline reporter from guessing protocol.
                    'goal_threshold': 0.10,
    ...
    def _map_actions_to_params(self, raw_action_batch, observations=None):
        ...
        TOTAL_W_BUDGET = self.parameter_layout.advection_scale
        MAX_K = self.parameter_layout.diffusion_bounds[1]
        BETA_MIN, BETA_MAX = self.parameter_layout.extra_bounds["beta"]
        ...
        # 공통 projector는 trainer와 동일한 unsmoothed target을 만든다. 아래의
        # critically damped filter는 Webots 실행 안정성을 위한 runtime 후처리이며
        # 그 차이는 PINN 감사 문서에 별도로 기록한다.
        projected = project_numpy_parameters(raw_action, self.parameter_layout)
        named_parameters = projected.as_named_dict(self.parameter_layout)
        flow_ratio_target = projected.omega[..., 0]
        k_target = named_parameters["k_diff"]
        beta_target = named_parameters["beta"]
    ...
    def _calculate_target_wheel_speeds(self, pde_params, observations):
        ...
        self._physics_navigation_shape_geometry = np.stack(
            (x_rel_global, y_rel_global, dynamic_R0), axis=-1
        ).astype(np.float32)
        ...
        # 공통 Navigation layout의 field 순서: goal flow, formation shape.
        self._physics_field_bases = np.stack(
            (V_global_flow, V_form_shape), axis=-2
        ).astype(np.float32)
            ...
            for n in range(self.num_envs):
                ...
                for i in range(self.num_agents):
                    ...
                    if self.phase_contract == "paper":
                        # S136의 four-stage state. The fixed arena geometry makes
                        # centroid x an observable event proxy for corridor entry/exit.
                        if not self.formation_reached_flag[n]:
                            phase_indicator[0] = 1.0  # formation keeping
                        elif true_centroid[0] < -0.7:
                            phase_indicator[1] = 1.0  # goal-directed navigation
                        elif true_centroid[0] < 0.6:
                            phase_indicator[2] = 1.0  # morphology adaptation
                        else:
                            phase_indicator[3] = 1.0  # formation recovery
                    elif centroid_dist_to_goal < 0.1 and self.formation_reached_flag[n]:
    ...
    def get_reward(self, pde_params, observations):
        ...
        for n in range(self.num_envs):
            ...
            phase_features = observations[n, :, 13 : 13 + self.num_phases]
            mask_phase_a = (phase_features[:, 0] > 0.5) & alive_mask
            if self.phase_contract == "paper":
                # Reward stages remain formation/transit/final-formation while
                # Macro-ADR distinguishes navigate and morph densities.
                mask_phase_b = (
                    (phase_features[:, 1] > 0.5)
                    | (phase_features[:, 2] > 0.5)
                ) & alive_mask
                mask_phase_c = (phase_features[:, 3] > 0.5) & alive_mask
            else:
