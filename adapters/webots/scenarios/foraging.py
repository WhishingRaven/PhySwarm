...
from adapters.webots import geometry as utilities
import math
import torch
...
PARAMETER_LAYOUT = get_parameter_layout("foraging", contract="legacy")


class Epuck2Supervisor(WebotsSupervisorEnv):
    def __init__(self, all_args=None):
        self.args = all_args
        self.parameter_layout = get_parameter_layout(
            "foraging",
            contract=getattr(self.args, "parameter_contract", "legacy"),
        )
        self.phase_contract = getattr(self.args, "phase_contract", "legacy")
        self.phase_names = get_runtime_macro_spec(
            "foraging", phase_contract=self.phase_contract
        ).phase_names
        ...
        self.trajectory_log_plan = TrajectoryLogPlan.from_options(
            scenario="foraging",
            save_trajectory=getattr(self.args, "save_trajectory", False),
            trajectory_output=getattr(self.args, "trajectory_output", None),
        ...
        # paper와 legacy는 모두 7채널이지만 transform/bounds가 다르다. 숫자를
        # 다시 적지 않고 선택된 계약이 actor space의 단일 기준이 되게 한다.
        self.num_actions = self.parameter_layout.action_dim
        ...
        #数据存储
        self.save_data = self.trajectory_log_plan.enabled
    ...
    def step(self, action):
        ...
        control_active_mask = self.alive_agent_buf.copy()
        # Macro-ADR는 actor observation에 없는 실제 arena 좌표를 사용한다. arena
        # origin을 빼면 병렬 Webots world가 모두 같은 고정 grid를 공유할 수 있다.
        control_positions = (
            self.agent_pos[:, :, :2] - self.arena_pos[:, None, :2]
        ).astype(np.float32, copy=True)
        control_phases = phase_indices_from_one_hot(
            control_observations[:, :, -self.num_phases :],
            active_mask=control_active_mask,
        )
        control_headings = self.agent_angle.copy()
        self._physics_field_bases = None
        pde_params = self._map_actions_to_params(action, control_observations)

        self._physics_reaction_gates = self._calculate_reaction_gates()
        ...
        if self.phase_contract == "paper":
            observations = self._advance_paper_phase_observations(observations)
        ...
        if self.save_data == True:
            self.save_test_data(pde_params, target_wheel_speeds, done)
        
        observation = ((observations, self.obs_neighbor_num_buf.copy()), state)
        info = {"per_env": reward_components_info}
        if self._physics_field_bases is not None:
            # 다음 observation의 body velocity는 방금 보낸 wheel command의 실제
            # Webots 결과다. 나머지 항은 command를 계산한 시점의 field다.
            next_active_mask = self.alive_agent_buf.copy()
            next_positions = (
                self.agent_pos[:, :, :2] - self.arena_pos[:, None, :2]
            ).astype(np.float32, copy=True)
            next_phases = phase_indices_from_one_hot(
                observations[:, :, -self.num_phases :],
                active_mask=next_active_mask,
            )
            info["physics"] = build_physics_transition(
                executed_velocity=observations[:, :, 0:2] * self.max_speed,
                field_bases=self._physics_field_bases,
                density=control_observations[:, :, 2],
                density_gradient=control_observations[:, :, 3:5],
                active_mask=control_active_mask & next_active_mask,
                field_bases_global=body_to_world_vectors(
                    self._physics_field_bases, control_headings
                ),
                position_start=control_positions,
                position_end=next_positions,
                phase_start=control_phases,
                phase_end=next_phases,
                active_start=control_active_mask,
                active_end=next_active_mask,
                reaction_gates=self._physics_reaction_gates,
            )
        return observation, reward, terminated, truncated, info

    def save_test_data(self, pde_params, target_wheel_speeds, done):
        for env_idx in range(self.num_envs):
            ...
            for i in range(self.num_agents):
                # 같은 control interval에 emitter로 보낸 wheel command를 직접
                # 기록한다. 존재하지 않는 cache의 fallback 0은 S174 smoothness를
                # 실제 움직임과 무관하게 0으로 만들 수 있다.
                l_speed = float(target_wheel_speeds[env_idx, i, 0])
                r_speed = float(target_wheel_speeds[env_idx, i, 1])
                # 提取标量值
                agent_log = {
                    ...
                    'episode_max_steps': int(self.episode_length),
                }
        ...
        if self.steps[0] >= self.episode_length or done[0].all():
            if self.episode_data_log:
                self.save_path = self.trajectory_log_plan.path_for_episode(self.index)
    ...
    def save_log_to_csv(self):
        if len(self.episode_data_log) > 0:
            ...
            df.to_csv(self.save_path, index=False, mode="x")

    def _calculate_reaction_gates(self):
        """pick/drop rate가 허용되는 위치·상태 gate ``chi``를 계산한다.

        논문 식 (7)의 반응률은 arena 전체에서 작동하지 않는다. food/nest의
        catch 영역과 현재 carrying state를 만족할 때만 각각 활성화된다.
        """
        ...
        gates = np.zeros(
            (
                self.num_envs,
                self.num_agents,
                len(self.parameter_layout.reaction_names),
            ),
            dtype=np.float32,
        )
        for n in range(self.num_envs):
            available_targets = self.target_capacities[n] > 0
            for i in range(self.num_agents):
                if not self.alive_agent_buf[n, i]:
                    continue
                position = self.agent_pos[n, i, :2]
                if self.agent_carrying_state[n, i]:
                    if self.phase_contract == "paper" and self.paper_phase_state[n, i] != 2:
                        continue
                    gates[n, i, 1] = float(
                        np.linalg.norm(position - self.nest_pos[n]) < self.catch_distance
                    )
                elif np.any(available_targets):
                    if (
                        self.phase_contract == "paper"
                        and self.paper_phase_state[n, i] not in {1, 3}
                    ):
                        continue
                    distances = np.linalg.norm(
                        self.target_pos[n][available_targets] - position, axis=1
                    )
                    gates[n, i, 0] = float(np.any(distances < self.catch_distance))
        return gates
    ...
    def update_carrying_state(self, pde_params):
        ...
        for n in range(self.num_envs):
            for i in range(self.num_agents):
                ...
                # --- 1. 未携粮状态：尝试拾取 ---
                if not is_carrying:
                    # Use the same spatial/phase gate recorded in PhysicsContext.
                    # This keeps the sampled simulator transition and lambda*chi
                    # reaction source on one contract.
                    if self._physics_reaction_gates[n, i, 0] <= 0.0:
                        continue
                    prob_pick = lambda_pick[n, i]
                    if getattr(self.args, "physics_loss_mode", "legacy") in {
                        "exact_macro",
                        "exact_joint",
                    }:
                        # exact modes interpret lambda as a continuous rate. The legacy
                        # checkpoint path keeps its historical per-step probability.
                        dt_seconds = float(self.timestep) / 1000.0
                        prob_pick = 1.0 - np.exp(-prob_pick * dt_seconds)
                ...
                # --- 2. 已携粮状态：尝试放下 ---
                    ...
                    if self._physics_reaction_gates[n, i, 1] <= 0.0:
                        continue
                    ...
                    if getattr(self.args, "physics_loss_mode", "legacy") in {
                        ...
                        dt_seconds = float(self.timestep) / 1000.0
                        prob_drop = 1.0 - np.exp(-prob_drop * dt_seconds)
    ...
    def _map_actions_to_params(self,raw_action_batch,observations=None,):
        """legacy checkpoint 출력을 이름 있는 물리 파라미터로 변환한다.

        이 경로와 trainer가 같은 parameter contract를 사용하므로 advection
        sparsification, diffusion 상한, reaction 변환이 더 이상 어긋나지 않는다.
        ``observations``는 과거 호출 계약을 보존하기 위한 인자이며 projection에는
        사용하지 않는다.
        """

        if torch.is_tensor(raw_action_batch):
            raw_action_batch = raw_action_batch.detach().cpu().numpy()
        projected = project_numpy_parameters(raw_action_batch, self.parameter_layout)
        return projected.as_named_dict(self.parameter_layout)
    ...
    def _calculate_target_wheel_speeds(self, pde_params, observations):
        ...
        # ParameterLayout 순서와 동일하게 저장해 trainer가 stochastic exploration
        # basis까지 정확히 재사용할 수 있게 한다.
        self._physics_field_bases = np.stack(
            (vec_food, vec_nest, vec_rand, vec_info), axis=-2
        ).astype(np.float32)
    ...
    def cleanup(self) -> None:
        ...
        # Paper mode uses a persistent S126 phase state. Reclassifying every
        # observation from instantaneous cues would create edges outside S129.
        self.paper_phase_state = np.zeros(
            (self.num_envs, self.num_agents), dtype=np.int64
        )
    ...
    def _advance_paper_phase_observations(self, observations):
        """Advance S129 once and make returned local/global state agree.

        ``get_observations`` is intentionally pure with respect to phase state
        because it is called both at the beginning and end of a control step.
        Only the post-dynamics call reaches this method, so a returned phase is
        exactly the next step's starting phase and the ``T+1`` physics payload
        remains contiguous.
        """

        self.paper_phase_state = advance_foraging_paper_phases(
            self.paper_phase_state,
            carrying=self.agent_carrying_state,
            resource_detected=observations[:, :, 7] > 0.05,
            trail_available=observations[:, :, 13] > 0.1,
            active_mask=self.alive_agent_buf,
        )
        phase_one_hot = np.eye(self.num_phases, dtype=np.float32)[
            self.paper_phase_state
        ]
        phase_one_hot *= self.alive_agent_buf[..., None]
        observations[:, :, -self.num_phases :] = phase_one_hot

        all_agent_obs = observations.reshape(self.num_envs, -1)
        self.states_buf = np.repeat(
            all_agent_obs[:, None, :], self.num_agents, axis=1
        ...
        if self.message is not None:
            for n in range(self.num_envs):
                ...
                for i in range(self.num_agents):
                    ...
                    phase_indicator = np.zeros(self.num_phases, dtype=np.float32)

                    if self.phase_contract == "paper":
                        # Persistent state is advanced once after dynamics. This
                        # read-only branch prevents two get_observations() calls
                        # at the same Webots state from producing two reactions.
                        phase_indicator[self.paper_phase_state[n, i]] = 1.0
                    else:
