...
PARAMETER_LAYOUT = get_parameter_layout("rescue", contract="legacy")
...
class Epuck2Supervisor(WebotsSupervisorEnv):
    def __init__(self, all_args=None):
        ...
        # 표준 task는 성공한 target을 count/reset한다. 과거 hard-coded True 때문에
        # 정상 학습도 robustness 측정 분기로 들어갔으므로 이제 명시적 flag만 쓴다.
        self.is_robust_test = bool(getattr(self.args, "robust_test", False))
    ...
    def step(self, action):
        ...
        # Rescue 관측의 마지막 값은 local rank이므로 phase는 11:14다.
        control_phases = phase_indices_from_one_hot(
            control_observations[:, :, 11:14], active_mask=control_active_mask
        )
        ...
        self._physics_reaction_gates = np.zeros(
            (self.num_envs, self.num_agents, len(PARAMETER_LAYOUT.reaction_names)),
            dtype=np.float32,
        )
        ...
        if self._physics_field_bases is not None:
            ...
            next_phases = phase_indices_from_one_hot(
                observations[:, :, 11:14], active_mask=next_active_mask
            )
            info["physics"] = build_physics_transition(
                ...
                density_gradient=control_observations[:, :, 9:11],
    ...
    def save_test_data(self, pde_params, target_wheel_speeds, done):
        for env_idx in range(self.num_envs):
            ...
            for i in range(self.num_agents):
                # Log the exact command sent during this interval. A missing
                # ``last_wheel_speeds`` attribute previously produced fake zeros.
                ...
                agent_log = {
                    ...
                    # S185 assigns failed trials T_max rather than dropping them.
    ...
    def update_role_and_task_state(self, pde_params):
        ...
        for n in range(self.num_envs):
            ...
            for i in range(self.num_agents):
                ...
                exact_reaction = getattr(self.args, "physics_loss_mode", "legacy") in {
                    "exact_macro",
                    "exact_joint",
                }
                if exact_reaction:
                    # Paper Eq. (7): independent non-negative rates with
                    # P(switch in dt)=1-exp(-lambda*dt). Only the current role's
                    # outgoing edge is activated by chi below.
                    dt_seconds = float(self.timestep) / 1000.0
                    prob_anchor = 1.0 - np.exp(
                        -float(lambda_anchor[n, i]) * dt_seconds
                    )
                    prob_release = 1.0 - np.exp(
                        -float(lambda_release[n, i]) * dt_seconds
                    )
                else:
                    # Existing checkpoints were trained with competing per-step
                    # signals and scenario-specific gains; preserve that contract.
                ...
                if not self.is_anchored[n, i]:
                    self._physics_reaction_gates[n, i, 0] = 1.0
                    if np.random.rand() < prob_anchor:
                        self.is_anchored[n, i] = True
                else:
                    self._physics_reaction_gates[n, i, 1] = 1.0
    ...
    def _map_actions_to_params(self, raw_action_batch, observations=None):
        """6채널 legacy Rescue action을 공통 호환 profile로 투영한다."""

        if torch.is_tensor(raw_action_batch):
            raw_action_batch = raw_action_batch.detach().cpu().numpy()
        projected = project_numpy_parameters(raw_action_batch, PARAMETER_LAYOUT)
        return projected.as_named_dict(PARAMETER_LAYOUT)
    ...
    def _calculate_target_wheel_speeds(self, pde_params, observations):
        ...
        # 6채널 legacy layout의 advection 순서: target, center, exploration.
        self._physics_field_bases = np.stack(
            (vec_target, vec_center, vec_rand), axis=-2
        ).astype(np.float32)
