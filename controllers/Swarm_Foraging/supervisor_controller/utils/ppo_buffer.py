...
class PPOBuffer(object):
    ...
    def compute_returns(self, idx, value_normalizer=None):
        ...
        for p_id in self.policy_info.keys():
            self.policy_buffers[p_id].compute_returns(idx, value_normalizer[p_id])
        return idx
...
class AdjPolicyBuffer(object):
    ...
    def sample_inds(self, data_chunk_length, num_mini_batch, value_normalizer):
        ...
        obs_neighbor_num = self.obs_neighbor_num[:-1].transpose(1, 0, 2, 3).reshape(batch_size, self.num_agents, -1)
        share_obs = self.share_obs[:-1].transpose(1, 0, 2, 3).reshape(batch_size, self.num_agents, -1)
        acts = self.acts.transpose(1, 0, 2, 3).reshape(batch_size, self.num_agents, -1)
        prob_acts = self.prob_acts.transpose(1, 0, 2, 3).reshape(batch_size, self.num_agents, -1)
        value_preds = self.value_preds[:-1].transpose(1, 0, 2, 3).reshape(batch_size, self.num_agents, -1)
        returns = self.returns[:-1].transpose(1, 0, 2, 3).reshape(batch_size, self.num_agents, -1)

        dones = np.concatenate((np.zeros((1, self.buffer_size, self.num_agents, 1)), self.dones), axis=0)
        dones = dones.transpose(1, 0, 2, 3).reshape((self.buffer_size, self.episode_length + 1, self.num_agents, 1))
        dones = dones[:, :-1].reshape(batch_size, self.num_agents, -1)

        dones_env = np.concatenate((np.zeros((1, self.buffer_size, 1)), self.dones_env), axis=0)
        dones_env = dones_env.transpose(1, 0, 2).reshape((self.buffer_size, self.episode_length + 1, 1))
        dones_env = dones_env[:, :-1].reshape(batch_size, -1)

        advantages = self.advantages 
        active_masks = 1 - self.dones 
