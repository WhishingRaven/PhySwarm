"""Shared MAPPO configuration with task-specific environment defaults."""

import argparse


def get_config(scenario="foraging"):
    task_defaults = {
        "foraging": dict(comm_view=0.4, collision_distance=0.12, catch_distance=0.25),
        "navigation": dict(comm_view=2.0, collision_distance=0.08, catch_distance=0.12),
        "rescue": dict(comm_view=0.6, collision_distance=0.12, catch_distance=0.15),
    }
    if scenario not in task_defaults:
        raise KeyError(f"unknown scenario {scenario!r}")
    task = task_defaults[scenario]
    ...
    parser.add_argument("--algorithm_name", type=str, default="rmatd3", choices=[
                       "qmix", "vdn", "matd3", "maddpg", "masac", "mqmix", "mvdn","rddfg_continuous","rddfg_cent_rw","mappo"])
    ...
    parser.add_argument("--pinn_coef", type=float, default=1.0,
                        help="Weight for the auxiliary PINN loss in actor update")
    parser.add_argument(
        "--parameter_contract",
        choices=("legacy", "paper"),
        default="legacy",
        help=("legacy preserves checked-in checkpoints; paper uses the bounded "
              "sigmoid manifold from Eq. S158 for new experiments"),
    )
    parser.add_argument(
        "--phase_contract",
        choices=("legacy", "paper"),
        default="legacy",
        help=("legacy keeps 3 checkpoint-compatible phase indicators; paper exposes "
              "explore/approach/home/trail as four explicit phases"),
    )
    parser.add_argument(
        "--physics_loss_mode",
        choices=("legacy", "exact_micro", "exact_macro", "exact_joint"),
        default="legacy",
        help=("legacy keeps the historical surrogate; exact_micro and exact_macro "
              "isolate one paper residual; exact_joint combines both"),
    )
    parser.add_argument("--macro_pinn_beta", type=float, default=1.0,
                        help="Weight beta for exact Macro-ADR inside the PINN loss")
    parser.add_argument("--macro_kde_bandwidth", type=float, default=0.1,
                        help="KDE bandwidth in metres for exact Macro-ADR")
    parser.add_argument("--max_pinn_loss", type=float, default=10.0,
                        help="Clamp value for the auxiliary PINN loss")
    parser.add_argument("--max_return_abs", type=float, default=200.0,
                        help="Absolute clamp for returns/value predictions before training")
    ...
    parser.add_argument("--constraint_lambda", type=float, default=0.2,
                        help="Initial Lagrange multiplier for RCPO safety constraint")
    parser.add_argument("--constraint_cost_threshold", type=float, default=0.2,
                        help="Safety cost limit threshold (RCPO constraint budget)")
    parser.add_argument("--constraint_lambda_lr", type=float, default=0.003,
                        help="Learning rate for updating the Lagrange multiplier lambda")
    parser.add_argument("--constraint_lambda_max", type=float, default=1.0,
                        help="Maximum clamp limit value for Lagrange multiplier lambda")
    ...
    parser.add_argument('--highest_orders', type=int, default=2, help="number of agents")
    ...
    parser.add_argument("--num_factor", type=int, default=3)
    parser.add_argument("--num_obs_targets", type=int, default=1)
    parser.add_argument("--num_obs_agents", type=int, default=3)
    parser.add_argument("--obs_view", type=float, default=0.5)
    parser.add_argument("--comm_view", type=float, default=task["comm_view"])
    parser.add_argument("--collision_distance", type=float, default=task["collision_distance"])
    parser.add_argument("--collision_reward", type=float, default=-1)
    parser.add_argument("--catch_distance", type=float, default=task["catch_distance"])
    ...
    parser.add_argument(
        ...
        default=False,
        help="Write one analysis-ready trajectory CSV per completed episode",
    )
    parser.add_argument(
        "--trajectory_output",
        type=str,
        default=None,
        help="CSV path (first episode) or directory for trajectory files",
    )
    parser.add_argument("--agent_loc_dim", type=int, default=14)
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        help="Keep a connected rescue target active for disturbance robustness evaluation",
    )
