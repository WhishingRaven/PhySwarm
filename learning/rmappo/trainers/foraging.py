"""Foraging physics-regularized MAPPO update strategy."""
...
from learning.trajectory_loss import exact_physics_loss_from_context
from physics.layouts import get_parameter_layout
from learning.rmappo.utils import soft_update, huber_loss, mse_loss, to_torch, log_loss, update_linear_schedule
from learning.rmappo.value_norm import ValueNorm
import numpy as np
...
class R_MAPPO:
    def __init__(self, args, num_agents, policies, policy_mapping_fn, device=torch.device("cpu"), episode_length=25,vdn=False):
        ...
        self.parameter_layout = get_parameter_layout(
            "foraging", contract=getattr(args, "parameter_contract", "legacy")
        )
        self.phase_contract = getattr(args, "phase_contract", "legacy")
        ...
        self.physics_loss_mode = getattr(self.args, 'physics_loss_mode', 'legacy')
        self.max_pinn_loss = float(getattr(self.args, 'max_pinn_loss', 10.0))
        self.macro_pinn_beta = float(getattr(self.args, 'macro_pinn_beta', 1.0))
        self.macro_kde_bandwidth = float(
            getattr(self.args, 'macro_kde_bandwidth', 0.1)
        )
        self.physics_dt = float(getattr(self.args, 'timestep', 32)) / 1000.0
        if self.macro_pinn_beta < 0.0 or self.macro_kde_bandwidth <= 0.0:
            raise ValueError("macro beta must be non-negative and bandwidth positive")
    ...
    def _calculate_pinn_loss(self, obs_batch, pde_params, active_masks):
        ...
        max_diffusion = self.parameter_layout.diffusion_bounds[1]
        k_diff_act = self._sanitize_tensor(
            pde_params['k_diff'].view(B, T, N, 1),
            clip_abs=max_diffusion,
        )
        ...
        if D == 18:
            # paper phase order adds trail at index 17. The historical surrogate
            # has no fourth equation, so its closest compatible target is the
            # information-guided exploration branch; exact_macro keeps it distinct.
            phase_search = phase_search + obs_batch[:, :, :, 17:18]
        ...
        # A has only two rows, so A^T A is rank-deficient before ridge
        # regularization.  The equivalent 2x2 dual solve uses only elementary
        # tensor operations and avoids a known MPS decomposition crash.
        sol_flat = ridge_least_squares_2row(
            A_flat, Y_flat, regularization=1.0e-4
        )
        ...
        target_k_diff = torch.clamp(
            sol[:, :, :, 4:5], 0.0, max_diffusion
        )
    ...
    def _torch_map_actions_to_params(self, raw_action_batch):
        """runtime과 동일한 선택형 projection을 gradient를 보존하며 적용한다."""

        layout = getattr(self, "parameter_layout", PARAMETER_LAYOUT)
        projected = project_torch_parameters(
            to_torch(raw_action_batch), layout
        )
        return projected.as_named_dict(layout)
    ...
    def train_policy_on_batch(self, batch, epoch=0, mini_batch_idx=0):
        ...
        returns_batch, value_preds_batch, physics_batch = batch
        ...
        for p_id in self.policy_ids:
            ...
            if self.physics_loss_mode.startswith('exact_'):
                if physics_batch is None:
                    raise RuntimeError(
                        f"{self.physics_loss_mode} requires the environment physics side-channel"
                    )
                exact = exact_physics_loss_from_context(
                    physics_batch,
                    pde_params_torch,
                    self.parameter_layout,
                    "foraging",
                    mode=self.physics_loss_mode,
                    dt=self.physics_dt,
                    phase_contract=self.phase_contract,
                    macro_weight=self.macro_pinn_beta,
                    bandwidth=self.macro_kde_bandwidth,
                    active_mask=active_masks_batch,
                )
                pinn_loss = soft_cap_nonnegative_loss(
                    exact.breakdown.total, maximum=self.max_pinn_loss
                )
                velocity_error = (
                    torch.norm(exact.micro_result.velocity_error, dim=-1).mean()
                    if exact.micro_result is not None
                    else exact.breakdown.total.detach() * 0.0
                )
                macro_rms = (
                    torch.sqrt(exact.macro_result.residual.square().mean())
                    if exact.macro_result is not None
                    else exact.breakdown.total.detach() * 0.0
                )
                pinn_logs = {
                    'pinn/loss_macro': exact.breakdown.macro.item(),
                    'pinn/loss_micro': exact.breakdown.micro.item(),
                    'pinn/loss_total_raw': exact.breakdown.total.item(),
                    'pinn/executed_edm_error': velocity_error.item(),
                    'pinn/macro_residual_rms': macro_rms.item(),
                    'pinn/mode_exact_micro': float(self.physics_loss_mode == 'exact_micro'),
                    'pinn/mode_exact_macro': float(self.physics_loss_mode == 'exact_macro'),
                    'pinn/mode_exact_joint': float(self.physics_loss_mode == 'exact_joint'),
                }
            else:
                pinn_loss, pinn_logs = self._calculate_pinn_loss(
                    obs_batch=obs_batch,
                    pde_params=pde_params_torch,
