"""Navigation physics-regularized MAPPO update strategy."""
...
class R_MAPPO:
    def __init__(self, args, num_agents, policies, policy_mapping_fn, device=torch.device("cpu"), episode_length=25,vdn=False):
        ...
        self.parameter_layout = get_parameter_layout(
            "navigation", contract=getattr(args, "parameter_contract", "legacy")
        )
    ...
    def _calculate_pinn_loss(self, obs_batch, pde_params, active_masks):
        ...
        raw_phases = obs_batch[:, :, :, 13:]
        if raw_phases.shape[-1] == 4:
            # Existing surrogate has gather/transit/settle equations. Map the
            # paper's navigate+morph stages to transit only for legacy mode;
            # exact_macro retains all four phase densities independently.
            phase_indicator = torch.cat(
                (
                    raw_phases[..., 0:1],
                    raw_phases[..., 1:2] + raw_phases[..., 2:3],
                    raw_phases[..., 3:4],
                ),
                dim=-1,
            )
        else:
            phase_indicator = raw_phases[..., :3]
        ...
        sol = detached_target_least_squares(A, Y)
    ...
    def _torch_map_actions_to_params(self, raw_action_batch):
        """4채널 Navigation target projection을 선택된 공통 계약으로 계산한다."""

        layout = getattr(self, "parameter_layout", PARAMETER_LAYOUT)
        projected = project_torch_parameters(
            to_torch(raw_action_batch), layout
        )
        values = projected.as_named_dict(layout)
        values["alpha"] = torch.ones_like(values["beta"])
        return values
