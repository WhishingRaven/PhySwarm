...
from learning.rmappo.networks.value import VFunction
from learning.rmappo.utils import get_dim_from_space, is_discrete, is_multidiscrete, make_onehot, DecayThenFlatSchedule, avail_choose, to_torch, to_numpy
from learning.rmappo.base.mlp_policy import MLPPolicy
from learning.rmappo.networks.recurrent import RNNBase
from learning.rmappo.networks.critic import critic_RNNBase
...
class R_MAPPOPolicy(MLPPolicy):
    ...
    def __init__(self, config, policy_config, train=True):
        action_head_class = config["action_head_class"]
        categorical_head_class = config["categorical_head_class"]
        ...
        if self.discrete_action:
            self.act = categorical_head_class(self.args, self.act_input_dim, self.act_dim, self.device)
        else:
            # self.act = DiagGaussian(self.args, self.act_dim, self.act_input_dim, self.act_dim, self.device)
            self.act = action_head_class(self.args, self.act_dim, self.act_input_dim, self.obs_dim, self.device)
