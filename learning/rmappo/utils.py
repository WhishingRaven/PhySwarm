"""Tensor, space, schedule, and optimization helpers for recurrent MAPPO."""
...
def update_linear_schedule(optimizer, epoch, total_num_epochs, initial_lr):
    """Decreases the learning rate linearly to a minimum value"""
    
    min_lr = 1e-5 
    progress = epoch / float(total_num_epochs)
    lr = min_lr + (initial_lr - min_lr) * (1.0 - progress)
    lr = max(lr, min_lr)
...
def gumbel_softmax_sample(logits, avail_logits, temperature, device=torch.device('cpu'),r_grad=False):
    ...
    gumbel_noise = -torch.log(-torch.log(uniform.clamp_min(1e-20)))
