import numpy as np
from learning.rollout_context import PhysicsContextBuffer
from learning.rmappo.utils import get_dim_from_space
import torch
...
class PPOBuffer(object):
    ...
    def insert(self, num_insert_episodes, obs, share_obs, acts, prob_acts, rewards, dones, dones_env,values=None,rnn_states=None,rnn_critic=None,avail_acts=None,obs_neighbor_num=None,physics_context=None):
        """
        ...
        for p_id in self.policy_info.keys():
            ...
            p_physics_context = physics_context.get(p_id) if physics_context is not None else None
...
class AdjPolicyBuffer(object):
    def __init__(self, buffer_size, episode_length, num_agents, obs_space, share_obs_space, act_space,
        ...
        # actor observation과 분리된 학습 전용 physics side-channel이다.
        self.physics_context = PhysicsContextBuffer(
    ...
    def insert(self, num_insert_episodes, obs, share_obs, acts, prob_acts, rewards, dones, dones_env,values=None,rnn_obs=None,rnn_share_obs=None,avail_acts=None,obs_neighbor_num=None,physics_context=None):
        ...
        self.physics_context.insert(physics_context, idx_range)
    ...
    def sample_inds(self, data_chunk_length, num_mini_batch, value_normalizer):
        ...
        for indices in sampler:
            ...
            physics_batch = self.physics_context.sample_chunks(
                [int(i) * data_chunk_length for i in indices],
                data_chunk_length,
            )


            yield obs_batch, obs_neighbor_num_batch, share_obs_batch, dones_batch, dones_env_batch, acts_batch, prob_acts_batch, advantages_batch, rnn_obs_batch, rnn_share_obs_batch, returns_batch, value_preds_batch, physics_batch
